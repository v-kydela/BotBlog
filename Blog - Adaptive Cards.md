...
When using the Bot Framework, clicking on a submit action will send a Bot Framework message activity to the bot. A `postBack` message's data will be contained in the activity's `value` property. Each input field in the card that sent the `postBack` will be represented as a property of that `value` object, and the properties will be named according to each input field's ID. Any properties already in the submit action's `data` property will also be present in the `value` object. For example, you might have an Adaptive Card like this:
